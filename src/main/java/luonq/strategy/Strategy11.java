...
//                    System.out.println();
                ...
                if (stockRatio.getRatioMap().containsKey((int) ratio)) {
                    stockToRatioMap.put(stock, ratio);
                }
            }

            List<String> stocks = stockToRatioMap.entrySet().stream().sorted((o1, o2) -> {
                double o1Double = o1.getValue();
                double o2Double = o2.getValue();
                int o1Int = (int) o1Double;
                int o2Int = (int) o2Double;

                StockRatio stock1Ratio = originRatioMap.get(o1.getKey());
                StockRatio stock2Ratio = originRatioMap.get(o2.getKey());
                Map<Integer, RatioBean> ratio1Map = stock1Ratio.getRatioMap();
                Map<Integer, RatioBean> ratio2Map = stock2Ratio.getRatioMap();

                RatioBean ratioBean1 = ratio1Map.get(o1Int);
                RatioBean ratioBean2 = ratio2Map.get(o2Int);

//                System.out.println(o1.getKey()+" "+ ratioBean1.avgGainRatio);
                if (ratioBean1.avgGainRatio < ratioBean2.avgGainRatio) {
                    return 1;
                }
                return -1;
            }).map(o -> o.getKey()).collect(Collectors.toList());
        ...
        for (Integer openR : openRange) {
            for (Double lossRange : lossRatioRange) {
                for (int i = 0; i < hitRatio.size(); i++) {
                    ...
                    if (hit != 0.7d || lossRange != 0.2d || openR != 7) {
                        continue;
                    }
