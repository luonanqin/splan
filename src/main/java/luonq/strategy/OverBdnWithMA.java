...
//                                                                        continue;
                    ...
                    for (int j = 0; j < dateList.size(); j++) {
                        ...
                        Map<String, StockKLine> prevStockKLineMap = Maps.newHashMap();
                        Map<String, MA> prevStockMAMap = Maps.newHashMap();
                        String lastDate = "";
                        if (j > 0) {
                            lastDate = dateList.get(j - 1);
                            lastStockBollMap = dateToStockBollMap.get(lastDate);
                            prevStockKLineMap = dateToStockLineMap.get(lastDate);
                            prevStockMAMap = dateToStockMAMap.get(lastDate);
                        }
                        ...
                        for (String stock : stocks) {
                            ...
                            BOLL prevBoll = lastStockBollMap.get(lastDate);
