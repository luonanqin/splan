...
public class Strategy14test {
    ...
    private Map<String, Map<String, Total>> _2023codeTotalMap = Maps.newHashMap();
    private Map<String, Map<String, StockKLine>> _2023codeLineMap = Maps.newHashMap();
    ...
    private void init() {
        ...
        for (List<Total> value : codeTotalMap.values()) {
            Collections.sort(value, Comparator.comparingInt(o -> BaseUtils.formatDateToInt(o.getDate())));
        }
        ...
        _2023codeLineMap = Maps.newHashMap();
        for (Total total : _2023) {
            String date = total.getDate();
            String code = total.getCode();
            StockKLine stockKLine = total.toKLine();

            Map<String, StockKLine> sMap = _2023codeLineMap.get(date);
            if (sMap == null) {
                sMap = Maps.newHashMap();
                _2023codeLineMap.put(date, sMap);
            }
            sMap.put(code, stockKLine);

            Map<String, Total> tMap = _2023codeTotalMap.get(date);
            if (tMap == null) {
                tMap = Maps.newHashMap();
                _2023codeTotalMap.put(date, tMap);
            }
            tMap.put(code, total);
        }
    }
    ...
    public void main() throws Exception {
        ...
        for (String code : allCode) {
            List<Total> totals = codeTotalMap.get(code);
            if (CollectionUtils.isEmpty(totals)) {
                continue;
            }
            Set<String> canTradeDate = totals.stream().filter(t -> {
                String line = t.getOpenTradeTime();
                return canTrade(line);
            }).map(Total::getDate).collect(Collectors.toSet());
    ...
    private boolean canTrade(String tradeTime) {
    ...
    public void predict(Map<String/* code */, StockRatio/* successRatio */> map) {
        List<String> dateList = _2023codeLineMap.keySet().stream().sorted(Comparator.comparingInt(BaseUtils::formatDateToInt)).collect(Collectors.toList());
        ...
        for (int i = 1; i < dateList.size(); i++) {
            ...
            Map<String, StockKLine> todayKline = _2023codeLineMap.get(today);
            Map<String, StockKLine> yesterdayKline = _2023codeLineMap.get(yesterday);
            Map<String, Total> totalMap = _2023codeTotalMap.get(today);
            for (String code : yesterdayKline.keySet()) {
                ...
                Total total = totalMap.get(code);
                if (total == null || !canTrade(total.getOpenTradeTime())) {
                    continue;
                }
...
//                    continue;
    ...
    static class RatioBean {
        ...
        public void add(Bean bean) {
            beanList.add(bean);
            long trueCount = beanList.stream().filter(c -> c.getOpen() > c.getPrevClose()).count();
