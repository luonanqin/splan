...
/**
 * 计算历史光脚阴线第二天上涨的概率（光脚阴线每个股票比例不一样），找成功率高的收盘前买入第二天收盘卖出（计算单只股票明细）
 */
@Component
public class Strategy15_1 {
    ...
    private void init() {
        codeData = readFromDB.getCodeDate("2023", "PHM", "asc");
    ...
    public void main() throws Exception {
        ...
        for (int i = 1; i < stockKLines.size() - 1; i++) {
            ...
            double closeOpenDiff = ((close / prevClose) - 1) * 100;
            ...
            if (close >= prevClose) {
                successCount++;
                System.out.println(date + "\tgain\t" + prevLowCloseDiff + "\t" + closeOpenDiff);
            } else {
                failCount++;
                System.out.println(date + "\tloss\t" + prevLowCloseDiff + "\t" + closeOpenDiff);
            }
        }
