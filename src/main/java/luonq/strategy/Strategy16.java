...
/**
 * 计算历史低于布林线下轨的百分比，按照0-6+多档计算当天收盘大于开盘的成功率，来进行预测。
 * 历史数据计算范围是当年的前两年，当年产生的新数据不作为预测使用
 * 备选股票根据各档位历史成功率倒排，取第一个进行交易
 * 测试结果：失败
 */
public class Strategy16 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int j = 0; j < dateList.size(); j++) {
            ...
            List<String> stocks = stockToRatioMap.entrySet().stream().sorted((o1, o2) -> {
                int int_o1 = (int) o1.getValue().doubleValue();
                int int_o2 = (int) o2.getValue().doubleValue();
                int_o1 = int_o1 > 6 ? 6 : int_o1;
                int_o2 = int_o2 > 6 ? 6 : int_o2;
                String stock1 = o1.getKey();
                String stock2 = o2.getKey();
                StockRatio stockRatio1 = originRatioMap.get(stock1);
                StockRatio stockRatio2 = originRatioMap.get(stock2);
                Map<Integer, RatioBean> ratioMap1 = stockRatio1.getRatioMap();
                double ratio1 = ratioMap1.get(int_o1).getRatio();
                Map<Integer, RatioBean> ratioMap2 = stockRatio2.getRatioMap();
                double ratio2 = ratioMap2.get(int_o2).getRatio();
                if (ratio1 < ratio2) {
                    return 1;
