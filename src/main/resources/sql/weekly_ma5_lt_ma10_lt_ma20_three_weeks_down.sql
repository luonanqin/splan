-- 连续三周「空头排列」：每周均线满足 ma5 < ma10 < ma20（短周期在下）。
-- 相邻两周（第1→2 周、第2→3 周）的增量满足：Δma5 < Δma10 < Δma20（均为后一周减前一周；下跌时常为负，短均线跌得更多）。
-- 与 weekly_ma5_gt_ma10_gt_ma20_three_weeks.sql 互为反向条件。
-- 仅统计 week_option 表中的标的（与 classpath option/weekOption 同步）。
-- 连续三周 = 同一 code 下按 date 排序的相邻三根周线。
